...
#Your offer [53] ? 0
  ...
  if ($in and /Equipment\s+Buying\s+(\d+)\s+(\d+)%/)           { $in=0 if ($1>3000 or $2<100); }
  if ($in and /Equipment do you want to sell \[(\d+)\]\?/)     { $holds=$1; }
  if ($in and /Equipment do you want to sell \[\d+\]\? (\d+)/) { $holds=$1; }
