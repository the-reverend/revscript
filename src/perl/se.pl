...
#Your offer [53] ? 0
  ...
  if ($in and /Equipment\s+Selling\s+(\d+)\s+(\d+)%/)         { $in=0 if ($1>3000 or $2<100); }
  if ($in and /Equipment do you want to buy \[(\d+)\]\?/)     { $holds=$1; }
  if ($in and /Equipment do you want to buy \[\d+\]\? (\d+)/) { $holds=$1; }
  if ($in and $holds>0 and /We'll sell them for (\d+) cred/)  { $price=$1/$holds; }

  # <P-Probe estimates your offer was  94.47% of best price>
  if ($in and /P-Probe estimates your offer was\s+(\d+(\.\d+)?)%/) { $price=$price*$1/100; }
