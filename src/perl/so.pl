...
#Your offer [53] ? 0
  ...
  if ($in and /Organics\s+Selling\s+(\d+)\s+(\d+)%/)          { $in=0 if ($1>3000 or $2<100); }
  if ($in and /Organics do you want to buy \[(\d+)\]\?/)      { $holds=$1; }
  if ($in and /Organics do you want to buy \[\d+\]\? (\d+)/)  { $holds=$1; }
